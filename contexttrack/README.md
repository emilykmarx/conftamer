...
```
$ go run ./contexttrack/client --output events.jsonl
```
...
**After running**, parse results using `analysis/group_by_context.py`. This will print a summary of all messages (sent and received) that share the same context (i.e., potential edges in our graph), including how many times this relationship was observed.
For example:
